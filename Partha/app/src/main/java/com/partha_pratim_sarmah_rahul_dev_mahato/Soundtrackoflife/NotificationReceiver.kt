...
class NotificationReceiver:BroadcastReceiver() {
    ...
    private fun prevNextSong(increment: Boolean, context: Context){
        if (PlayerActivity.shuffleMode && !PlayerActivity.repeat) {
            PlayerActivity.songPosition = getRandom(PlayerActivity.musicListPA.size - 1)
        } else if (!PlayerActivity.shuffleMode && !PlayerActivity.repeat) {
            PlayerActivity.songPosition = (PlayerActivity.songPosition) % PlayerActivity.musicListPA.size
        }
